...
export function registerRuntimeConsole(app: Express, config: CodexProConfig, workspaces: WorkspaceManager, runtime: Runtime) {
  ...
  app.get("/runtime/state", async (req, res) => {
    try {
      ...
      res.json(redactStructured({ workspaces: workspaces.listWorkspaces(), ...(await tasks.context(workspace)), tasks: tasks.list(workspace).slice(0, 30) }));
  ...
  app.post("/runtime/action", express.json({ limit: "4kb" }), (req, res) => {
    try {
      ...
      res.json(body.action === "cancel_job" ? runtime.cancelJob(workspace, body.id) : runtime.closeShell(workspace, body.id));
    } catch (error) { res.status(400).json({ error: redactSensitiveText(String(error)) }); }
  });
}
...
const $=id=>document.getElementById(id);let selected='',logJob=null;
...
async function refresh(){try{const data=await request('/runtime/state'+(selected?'?workspace_id='+encodeURIComponent(selected):''));selected=data.workspace.id;
...
$('git').textContent=data.branch+' / '+data.head+'\\n'+data.git_status+'\\n'+data.diff_summary;$('validation').textContent=Object.keys(data.validation).length?JSON.stringify(data.validation,null,2):'No test results recorded.';$('handoff').textContent=data.handoff.text||'No agent handoff recorded.';$('error').textContent='';$('updated').textContent='Updated '+new Date().toLocaleTimeString();if(logJob)await logs(logJob.id,logJob.stream)
}catch(e){$('error').textContent=e.message}}
$('workspace').onchange=()=>{selected=$('workspace').value;logJob=null;refresh()};async function poll(){await refresh();setTimeout(poll,3000)}poll();
</script></html>`;
